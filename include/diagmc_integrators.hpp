...
template <typename T>
class mt_mcmc_gw_mf_integrator {
 private:
 ...
 public:
  ...
  std::optional<lattice_2d_b_interp> w_r_tau;
  ...
  // Constructor
  mt_mcmc_gw_mf_integrator(const hub_2dsqlat_mcmc_config &cfg_,
                                   const lattice_2d_f_interp &g_r_tau_,
                                   const diagram_pools_el &ss_diags_,
                                   const hc_lat_mf_coords &mf_meas_coords_,
                                   const meas_t &meas_sums_)
      // Parent constructor
      : cfg(cfg_),
        g_r_tau(g_r_tau_),
        ss_diags(ss_diags_),
  ...
  double dn_weight() {
    ...
    for (int i_diag = 0; i_diag < diags.n_diags; ++i_diag) {
      ...
      for (edge_t edge : ferm_edges) {
        ...
        this_weight *= g_r_tau(del_nr[0], del_nr[1]).ap_eval(del_tau);
      }
      if (using_local_wf) {
    ...
    for (int i_diag = 0; i_diag < diags.n_diags; ++i_diag) {
      ...
      for (std::size_t i = 0; i < 2; ++i) {  // (coords.size() = 2)
        ...
        for (edge_t f_edge : f_edges) {
          ...
          // Evaluate the Green's function interpolant over
          // the spacetime distance associated with this line
          ferm_io_weight *= g_r_tau(del_nr[0], del_nr[1]).ap_eval(del_tau);
        }
