...
// Configuration object for momentum-space MCMC for the Hubbard model on the 2d
...
class mb_rt_mcmc_2d_sq_hub_mf_integrator {
 private:
  Rand_engine rand_gen;
  Unif_gen std_uniform;
  DUnif_gen roll_1d3;
  DUnif_gen select_modifiable;
  DUnif_gen band_index_gen;
 ...
 public:
  ...
  std::vector<int> band_idxs_curr;
  std::vector<int> band_idxs_prop;
  hub_2dsqlat_mcmc_config cfg;
  lat_st_coords coords_curr;
  lat_st_coords coords_prop;
  mb_lattice_2d_f_interp g_r_tau;
  ...
  // Constructor
  mb_rt_mcmc_2d_sq_hub_mf_integrator(const hub_2dsqlat_mcmc_config &cfg_,
                                     const mb_lattice_2d_f_interp &g_r_tau_,
                                     const diagram_pools_el &ss_diags_,
                                     const lat_mf_coords &mf_meas_coords_, const meas_t &meas_sums_)
      // Parent constructor
      : cfg(cfg_),
        g_r_tau(g_r_tau_),
        ss_diags(ss_diags_),
    ...
    if (using_local_wf) {
      ...
      for (int i = 1; i < n_subspaces; i++) {
        std::vector<double> row;
        for (int j = 0; j < ss_diags[i].n_diags; j++) {
          row.push_back(0.0);
        }
    ...
    std::unordered_map<std::string, int> dt_map = {
    ...
    // For generating internal band indices
    band_index_gen.param(DUnif_gen::param_type(0, cfg_.phys.n_band.value_or(1) - 1));
  ...
  meas_t results(bool normalize = true) {
    if (normalize) {
      ...
      for (int i_ss = 1; i_ss < n_subspaces; ++i_ss) {
        for (std::size_t i_meas = 0; i_meas < meas_means[i_ss].size(); ++i_meas) {
  ...
  void summarize(bool to_file = false) const {
    ...
    for (std::size_t i = 0; i < coords_curr.size(); i++) {
  ...
  constexpr void setup(const int idx_ss_init, const lat_st_coords &initial_coords,
                       const std::vector<int> &initial_band_idxs) {
    ...
    if (idx_ss_init == 0) {
      ...
      band_idxs_prop = initial_band_idxs;
  ...
  // Seed new spacetime proposal coordinates for vertices in the exclusive range [v_start, v_stop)
  int seed_coords_prop(int v_start, int v_stop) {
    ...
    for (int i = v_start; i < v_stop; ++i) {
      ...
      if (constraints[i] == -1) {
        ...
        // Seed the center-of-mass coordinate (using COM constructor format)
        if (i == 0) {
          coords_prop.push_back(lat_st_coord(cfg.phys.dim, cfg.phys.n_site_pd, cfg.phys.beta,
        ...
        // Seed a regular coordinate object and add it to the list
        coords_prop.push_back(lat_st_coord(v_seed_id, v_seed_itime, v_seed_posn, cfg.phys.n_site_pd,
                                           cfg.phys.beta, cfg.propr.delta_tau, cfg.phys.lat_const,
                                           debug));
        ++n_seeded;
      }
      ...
      // interaction, constrained vertices differ only from their partners by ID#
        ...
        lat_st_coord v_constr = coords_prop[constraints[i]];
        // Update the ID# of the st-constrained vertex (differs from mate ID)
        v_constr.id = i;
        // Add the constrained vertex to the list of coordinates
  ...
  void ascend() {
    ...
    // Bands are spin-like for this theory
    int n_new_band_idxs = ss_diags[idx_ss_prop].n_spins_max - ss_diags[idx_ss_curr].n_spins_max;
    ...
    proposal_ratio = std::pow(cfg.phys.n_band.value_or(1), n_new_band_idxs) *
                     std::pow(cfg.phys.beta, n_new_times);
    ...
    // Generate the maximum number of new bands
    band_idxs_prop = band_idxs_curr;
    for (int i = 0; i < n_new_band_idxs; ++i) {
      band_idxs_prop.push_back(band_index_gen(rand_gen));
    }
  ...
  void descend() {
    ...
    int n_verts_prop = ss_diags[idx_ss_prop].n_verts;
    // Bands are spin-like for this theory
    int n_rem_bands = ss_diags[idx_ss_curr].n_spins_max - ss_diags[idx_ss_prop].n_spins_max;
    int n_bands_prop = 2 * subspaces[ss_diags[idx_ss_prop].n_spins_max];
    ...
    proposal_ratio = std::pow(1.0 / static_cast<double>(cfg.phys.n_band.value_or(1)), n_rem_bands) *
                     std::pow(1.0 / cfg.phys.beta, n_rem_times);
    // There is one extra time for outgoing vertex of polarization diagrams
    ...
    // Truncate the list of spacetime coordinates to the number of vertices in the proposed order
    coords_prop = coords_curr;
    coords_prop.resize(n_verts_prop);
    // Truncate the list of proposed bands to the number in the proposed order
    band_idxs_prop = band_idxs_curr;
    band_idxs_prop.resize(n_bands_prop);
    // Update the proposal diagram weight
    weight_prop = get_weight_prop();
    if (debug) {
      std::cout << "weight_prop = " << weight_prop << std::endl;
  ...
  // Propose to update one of the diagram internal variable(s) (while preserving the subspace)
  void mutate() {
    if (debug) {
    ...
    // Bands are spin-like for this theory
    int n_bands_max_prop = ss_diags[idx_ss_prop].n_spins_max;
    // Total number of modifiables, for selecting internal variable(s) to alter
    int n_modifiables = n_bands_max_prop + (cfg.phys.dim + 1) * modifiables.size();
    ...
    if (selection < n_bands_max_prop) {
      if (debug) {
        std::cout << "Reseeding band " << (selection + 1) << "/" << n_bands_max_prop << std::endl;
      }
      band_idxs_prop[selection] = band_index_gen(rand_gen);
      // We randomly select 1 of n_band possible configurations for the new band index
      proposal_ratio = 1.0 / static_cast<double>(cfg.phys.n_band.value_or(1));
    }
    // Reseed the selected time (clean update); this leaves the state probability unchanged
    else if (selection < n_bands_max_prop + n_times) {
      // Shift the selection to index times
      selection = selection - n_bands_max_prop;
    ...
    // Move the selected position index to a nearest neighbor
      ...
      selection = selection - n_bands_max_prop - n_times;
  ...
  void measure() {
    ...
    //       which need not be true for a general lattice problem...
      ...
      const lat_st_coord &v_out_rt = coords_curr[v_meas_ext];
      ...
      for (std::size_t j = 0; j < meas_sums[idx_ss_curr].size(); ++j) {
        ...
        const lat_mf_coord &v_out_mf_j = mf_meas_coords[j];
        // The FT factor is: e^{-i (p * r - nu * tau)}
        p_dot_r = 0.0;
        nu_tau = v_out_mf_j.imfreq * v_out_rt.itime;
        for (int k = 0; k < cfg.phys.dim; ++k) {
  ...
  int propose() {
    ...
    int selection = roll_1dn<3>(rand_gen);
  ...
  double dn_weight() {
    ...
    for (int i_diag = 0; i_diag < diags.n_diags; ++i_diag) {
      ...
      double weight_extra_band_sums =
          std::pow(cfg.phys.n_band.value_or(1), diags.n_spins_max - diags.n_loops[i_diag]);
      double this_weight = std::pow(-1.0, diags.n_loops[i_diag]) / weight_extra_band_sums;
      ...
      for (edge_t f_edge : ferm_edges) {
        // std::cout << "Edge: [" << f_edge[0] << ", " << f_edge[1] << "]" << std::endl;
        // Look up the band for this edge in the list of current diagram loops;
        // since the theory is spin-like, the start/end band index are the same.
        int band_index;
        const std::vector<std::vector<int>> &loops_this_graph = diags.loops[i_diag];
        for (int i_loop = 0; i_loop < diags.n_loops[i_diag]; ++i_loop) {
          for (int v : loops_this_graph[i_loop]) {
            // Found the vertex in this fermion loop; grab its band index and break
            if ((v == f_edge[0]) || (v == f_edge[1])) {
              band_index = band_idxs_prop[i_loop];
              // std::cout << "Found band index b = " << band_index << std::endl;
              break;
            }
          }
        }
        ...
        std::tie(del_nr, del_tau) = coords_prop[f_edge[1]] - coords_prop[f_edge[0]];
        this_weight *= g_r_tau({band_index, band_index, del_nr[0], del_nr[1]}).ap_eval(del_tau);
      }
      if (using_local_wf) {
  ...
  double dn_weight_mutate(int v_modif) {
    ...
    for (int i_diag = 0; i_diag < diags.n_diags; ++i_diag) {
      ...
      const std::array<lat_st_coords, 2> &coords{coords_curr, coords_prop};
      for (std::size_t i = 0; i < 2; ++i) {  // (coords.size() = 2)
        ...
        for (edge_t f_edge : f_edges) {
          ...
          // the spacetime distance associated with this line
          ferm_io_weight *=
              g_r_tau({band_index, band_index, del_nr[0], del_nr[1]}).ap_eval(del_tau);
        }
...
// square lattice; it is assumed the measurement is a scalar, or a one-particle
// correlation function that is a function of an external Matsubara 4-vector.
class rt_mcmc_cfg_2d_sq_hub_mf_meas {
 private:
  Rand_engine rand_gen;
  DUnif_gen select_modifiable;
  Discr_gen posn_shift_gen;
 ...
 public:
  ...
  meas_t meas_means;  // normalized mean data
  // Constructor
  rt_mcmc_cfg_2d_sq_hub_mf_meas(const hub_2dsqlat_mcmc_config &cfg_,
                                const lattice_2d_f_interp &g_r_tau_,
                                const diagram_pools_el &ss_diags_,
                                const lat_mf_coords &mf_meas_coords_, const meas_t &meas_sums_)
      // Parent constructor
      : cfg(cfg_),
    ...
    // For selecting internal variable(s) to alter; the total number
    // of modifiables is: (d+1)*n, where n is the subspace order
  ...
  int seed_coords_prop(int v_start, int v_stop) {
    ...
    for (int i = v_start; i < v_stop; ++i) {
      ...
      if (constraints[i] == -1) {
        ...
        lat_st_coord v_seed(v_seed_id, v_seed_itime, v_seed_posn, cfg.phys.n_site_pd, cfg.phys.beta,
                            cfg.propr.delta_tau, cfg.phys.lat_const, debug);
  ...
  double dn_weight() {
    ...
    for (int i_diag = 0; i_diag < diags.n_diags; ++i_diag) {
      ...
      for (edge_t f_edge : ferm_edges) {
        ...
        std::vector<int> del_nr(cfg.phys.dim);
        std::tie(del_nr, del_tau) = coords_prop[f_edge[1]] - coords_prop[f_edge[0]];
        this_weight *= g_r_tau(del_nr).ap_eval(del_tau);
      }
      if (using_local_wf) {
        // Record the full weight of this diagram, with prefactors
        ss_diag_weights_prop[idx_ss_prop][i_diag] = prefactor * this_weight;
    ...
    for (int i_diag = 0; i_diag < diags.n_diags; ++i_diag) {
      ...
      for (std::size_t i = 0; i < 2; ++i) {  // (coords.size() = 2)
        ...
        for (edge_t f_edge : f_edges) {
          ...
          // Evaluate the Green's function interpolant over
          // the spacetime distance associated with this line
          ferm_io_weight *= g_r_tau(del_nr).ap_eval(del_tau);
        }
        // Fermionic part of the 3-vertex weight
        vert_weights[i] = ferm_io_weight;
      }
...
// square lattice; it is assumed the measurement is a scalar, or a one-particle
// correlation function that is a function of an external Matsubara 4-vector.
class mt_mcmc_cfg_2d_sq_hub_mf_meas {
 private:
  Rand_engine rand_gen;
  Unif_gen std_uniform;
  DUnif_gen select_modifiable;
 ...
 public:
  ...
  meas_t meas_means;  // normalized mean data
  // Constructor
  mt_mcmc_cfg_2d_sq_hub_mf_meas(const hub_2dsqlat_mcmc_config &cfg_,
                                const lattice_2d_f_interp &g_r_tau_,
                                const diagram_pools_el &ss_diags_,
                                const lat_mf_coords &mf_meas_coords_, const meas_t &meas_sums_)
      // Parent constructor
