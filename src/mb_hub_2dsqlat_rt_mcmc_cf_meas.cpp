...
void update_yaml_config(const hub_2dsqlat_mcmc_config& cfg,
  ...
  std::unordered_map<std::string, std::string> param_updates = {
...
bool consistent_propagator_data(const std::string& h5_filename, const json& config,
  ...
  H5std_string cfg = hdf5::read_attribute<std::string, H5::H5File>("config", file);
  if (verbose) {
...
mb_lattice_2d_f_interp load_greens_function_h5(const std::string& h5_filename,
  ...
  hdf5::verify_attribute_type<double>("lat_g0_rt_data", datatype);

  // Get rank, dimensions and size of the data
  int g0_rank = dataspace.getSimpleExtentNdims();
  std::vector<hsize_t> dims_g0(g0_rank);
  dataspace.getSimpleExtentDims(dims_g0.data(), NULL);
...
void aggregate_and_save(int mpi_size, int mpi_rank, int mpi_main,
  ...
  if (is_main_thread) {
    ...
    hdf5::write_attribute<std::string>("config", static_cast<json>(cfg).dump(), h5file);

    // Save extra MCMC parameters as HDF5 attributes
    hdf5::write_attribute<bool>("normalized", normalized, h5file);
    hdf5::write_attribute<int>("n_ascend", integrator.n_ascend, h5file);
    hdf5::write_attribute<int>("n_descend", integrator.n_descend, h5file);
    hdf5::write_attribute<int>("n_mutate", integrator.n_mutate, h5file);
    hdf5::write_attribute<int>("max_order", integrator.max_order, h5file);
    hdf5::write_attribute<int>("v_meas_ext", integrator.v_meas_ext, h5file);
      ...
      for (const auto& nu : nu_list) {
        for (const auto& nk_vec : path_nk_coords) {
          mf_meas_coords.push_back(
              lat_mf_coord(id, nu, nk_vec, cfg.phys.n_site_pd, cfg.phys.lat_const));
          ++id;
        }
      }
      assert(n_meas_coords == static_cast<int>(mf_meas_coords.size()));
      if (debug) {
